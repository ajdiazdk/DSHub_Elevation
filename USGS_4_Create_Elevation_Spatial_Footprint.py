...
import os, subprocess, sys, traceback, re, glob, math, time, fnmatch
...
def mergeFootPrints(outDir,shapefiles):
    ...
    try:
        ...
        del daShapefile,dataSource,daLayer,inLayerDefn
...
#### ===================================================================================
def mergeFootPrints_OLD(outDir,shapefiles):

    try:
        AddMsgAndPrint(f"\nMerging Shapefile Spatial Footprints")

        # Get the list of shapefiles to merge from outDir
        shapefiles = [os.path.join(outDir, file) for file in os.listdir(outDir) if file.endswith(".shp") \
                      and file.find('WGS84') == -1 and file.find(f"USGS_3DEP_{resolution}_FootPrints") == -1]
        ...
        """ ------------------  Reproject shapefiles to 4326 ----------------------------------"""
        AddMsgAndPrint(f"\n\tProjecting {len(shapefiles)} Shapefile footprints to WGS84")
        projectedShapes = list()
        for shapefile in shapefiles:
            print(f"\t\tProjecting: {os.path.basename(shapefile)} to WGS84")

            inDriver = ogr.GetDriverByName("ESRI Shapefile")
            shape = inDriver.Open(shapefile,0)
            layer = shape.GetLayer()
            ...
            # projected Shapefile
            outShape = f"{shapefile.split('.')[0]}_WGS84.shp"

            # Delete if projected shp exists
            if os.path.exists(outShape):
                outDriver.DeleteDataSource(outShape)

            prjCmd = f"ogr2ogr -f \"ESRI Shapefile\" {outShape} {shapefile} -s_srs EPSG:{epsg} -t_srs EPSG:4326 -lco ENCODING=UTF-8"

            execCmd = subprocess.Popen(prjCmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE,universal_newlines=True)
            msgs, errors = execCmd.communicate()
            errors = ','.join(errors.strip().split('\n'))  # some errors return mutliple carriage returns

            projectedShapes.append(outShape)

            # Close datasource
            shape = None
            del inDriver,shape,layer,srs,epsg,outShape

        AddMsgAndPrint(f"\t\tSuccessfully Projected {len(projectedShapes):,} footprint Shapefiles to WGS84")

        """ ------------------  Append projected shapefile to merge shapefile -------------------------------"""
        AddMsgAndPrint(f"\n\tAppending {len(projectedShapes)} to {mergeShapePath}")
        appendCnt = 0

        # Add each shapefile to the merged layer
        for shapefile in projectedShapes:
            print(f"\t\tAppending: {os.path.basename(shapefile)}")

            # Layer of Target Shp
            #outDriver = ogr.GetDriverByName("ESRI Shapefile")
            #ds_target = outDriver.Open(mergeShapePath, 1)
            #targetLyr = ds_target.GetLayer()
            #targetDefn = targetLyr.GetLayerDefn()
        ...
        AddMsgAndPrint(f"\t\tSuccessfully Appended {appendCnt:,} footprint Shapefiles to: {os.path.basename(mergeShapePath)}")

            #ds_target.FlushCache()
            #ds_target.Destroy()

            # Close the merged shapefile
            #ds_target = None
            #ds_source = None

            #del ds_source, targetLyr,targetDefn

        """ ------------------  Delete Temp Projected Files -------------------------------"""
        AddMsgAndPrint(f"\n\tDeleting {len(projectedShapes)} Projected Shapefiles")
        deleteCnt = 0

        # Add each shapefile to the merged layer
        for shapefile in projectedShapes:
            outDriver.DeleteDataSource(shapefile)
            print(f"\t\tSuccessfully Deleted {os.path.basename(shapefile)}")
            deleteCnt+=1
...
if __name__ == '__main__':
    ...
    elevationMetadataFile = r'E:\GIS_Projects\DS_Hub\Elevation\DSHub_Elevation\USGS_Text_Files\30M\20230801_windows\USGS_3DEP_30M_Step2_Elevation_Metadata.txt'
    outFpDir = r'D:\projects\DSHub\USGS_Spatial_Footprints\30M'
    outputSRS = '4326'

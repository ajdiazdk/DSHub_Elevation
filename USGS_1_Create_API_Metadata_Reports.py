...
if __name__ == '__main__':
    ...
    try:
        ...
        for row in arcpy.da.SearchCursor(hucBoundaries, [hucCodeFld,hucNameFld], sql_clause=(None, orderByClause)):
            ...
            # JSON results
                ...
                if results['total'] > 0:
                    ...
                    f.write(f"\n{hucDigit},{hucName},{totalNumOfTiles},{tnmURLhuc}")
        ...
        if numOfTotalTiles > numOfQualifiedTiles:
            ...
            AddMsgAndPrint(f"Total # of Overlap Elevation Files: {(numOfTotalTiles - numOfQualifiedTiles):,}")
            AddMsgAndPrint(f"Total # of Unique Elevation Files to download: {numOfQualifiedTiles:,}")
            AddMsgAndPrint(f"\nTotal Download Size (According to USGS Metadata): {convert_bytes(totalSizeBytes)}")

        else:
            AddMsgAndPrint(f"Total # of Elevation Files: {numOfQualifiedTiles:,}")
